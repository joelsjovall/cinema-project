...
public static class MovieRoutes
{
  ...
  public static void Start()
  {
    ...
    App.MapPost("/movies/screenings/{id}/book", async (HttpContext context, int id, JsonElement bodyJson) =>
{
  ...
  try
  {
    ...
    var screeningSalonId = await screeningCommand.ExecuteScalarAsync();
    ...
    if (!SalonSeatLayouts.TryGetValue(salonId, out var layout))
    ...
    foreach (var seatNumber in selectedSeatNumbers)
    {
      if (!TryToRowSeat(layout, seatNumber, out var rowNumber, out var seatNumberInRow))
        return RestResult.Parse(context, Obj(new { error = $"Seat {seatNumber} does not exist." }));
      ...
      var existingSeatId = await seatLookupCommand.ExecuteScalarAsync();
      ...
      if (existingSeatId == null)
      {
        ...
        await createSeatCommand.ExecuteNonQueryAsync();
    ...
    var occupiedCount = Convert.ToInt32(await occupiedCheckCommand.ExecuteScalarAsync());

    if (occupiedCount > 0)
      return RestResult.Parse(context, Obj(new { error = "One or more seats are already booked." }));

    bookingCode = "B" + Random.Shared.Next(100000, 999999);
    ...
    insertBookingCommand.Parameters.AddWithValue("@userId", userIdToStore ?? (object)DBNull.Value);
    ...
    await insertBookingCommand.ExecuteNonQueryAsync();
    bookingId = (int)insertBookingCommand.LastInsertedId;
    ...
    foreach (var seatId in selectedSeatIds)
    {
      ...
      await insertBookingSeatCommand.ExecuteNonQueryAsync();
    }
